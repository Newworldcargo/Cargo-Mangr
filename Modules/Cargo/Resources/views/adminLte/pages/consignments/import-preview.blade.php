    ...
    <form id="mappingForm" method="POST" action="{{ route('consignment.import.preview.update', $batch->uuid) }}">
        ...
        <div class="card mb-3"><div class="card-header"><strong>2. Consignment details</strong><span class="text-muted ml-2">These values apply to the whole uploaded table.</span></div><div class="card-body">
            <div class="row">
                ...
                <div class="col-md-4 form-group"><label>Operational branch <span class="text-danger">*</span></label><select name="branch_id" class="form-control"><option value="">Choose branch</option>@foreach($branches as $branch)<option value="{{ $branch->id }}" {{ $batch->branch_id == $branch->id ? 'selected' : '' }}>{{ $branch->name }}</option>@endforeach</select></div>
            </div>
            <div class="row">
                <div class="col-md-3 form-group"><label>Origin country <span class="text-danger">*</span></label><select name="from_country_id" class="form-control"><option value="">Select</option>@foreach($countries as $country)<option value="{{ $country->id }}" {{ $batch->from_country_id == $country->id ? 'selected' : '' }}>{{ $country->name }}</option>@endforeach</select></div>
                <div class="col-md-3 form-group"><label>Origin state <span class="text-danger">*</span></label><select name="from_state_id" class="form-control"><option value="">Select</option>@foreach($states as $state)<option value="{{ $state->id }}" {{ $batch->from_state_id == $state->id ? 'selected' : '' }}>{{ $state->name }}</option>@endforeach</select></div>
                <div class="col-md-3 form-group"><label>Destination country <span class="text-danger">*</span></label><select name="to_country_id" class="form-control"><option value="">Select</option>@foreach($countries as $country)<option value="{{ $country->id }}" {{ $batch->to_country_id == $country->id ? 'selected' : '' }}>{{ $country->name }}</option>@endforeach</select></div>
                <div class="col-md-3 form-group"><label>Destination state <span class="text-danger">*</span></label><select name="to_state_id" class="form-control"><option value="">Select</option>@foreach($states as $state)<option value="{{ $state->id }}" {{ $batch->to_state_id == $state->id ? 'selected' : '' }}>{{ $state->name }}</option>@endforeach</select></div>
            </div>
        </div></div>
